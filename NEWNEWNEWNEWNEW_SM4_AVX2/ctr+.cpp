...
//注意函数形参
unsigned int SM4_CTRPLUS(__m256i NONCE96[32], __m256i key[32][8], __m256i over[32])
{
    //Packing...
    ...
    //trick3：避免编译器优化
    __m256i and8 = _mm256_set_epi64x(0x00000000000000FF, 0x000000FF00000000, 0x0000000000000000, 0xFF000000000000FF);
    ct[10] = _mm256_and_si256(ct[10], and8);
    unsigned int* extract = (unsigned int*)&ct[10];
    unsigned int back3 = extract[0];
    //printf("\n\n %ud\n\n", extract[0]);
    return back3;
}
