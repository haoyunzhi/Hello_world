...
//注意函数形参
unsigned int SM4_GCM(__m128i IV, __m256i NONCE96[32], __m256i key[32][8], __m256i over[32], unsigned long ulInitialInput[4], unsigned long ulKeyList1[32], unsigned long ulKeyList2[32])
{
    //Packing...
    ...
    /*
    //trick：避免编译器优化掉 bit transformation 2
    __m128i and8 = _mm_set_epi32(0x00000000, 0x00000000, 0x00000000, 0xFF000000);
    __m128i ct63 = _mm_and_si128(ct[63], and8);
    unsigned int* extract = (unsigned int*)&ct63;
    ...
    */
    ...
    //trick4：避免编译器优化掉 bit transformation 2
    __m128i and8 = _mm_set_epi32(0x00FF0000, 0x00000000, 0x00000000, 0xFF000000);
    Y = _mm_and_si128(Y, and8);
    unsigned int* extract = (unsigned int*)&Y;
    unsigned int back4 = extract[0];
